...
print("🚀 启动全维量化大脑 V6.0 (新浪引擎兜底版 + 游资全网扫描)...")
...
# ==========================================
# 🌪️ 1. 终极王牌大漏斗：调用新浪财经全网接口 (Sina)
# ==========================================
def get_sina_active_pool(top_n=250):
    print("正在打出最后一张王牌：调用新浪财经接口执行全市场漏斗降维...")
    try:
        # 新浪接口对海外 IP 容忍度略高
        spot_df = robust_akshare_call(ak.stock_zh_a_spot)
        
        # 清洗基础数据 (新浪接口使用英文列名)
        spot_df = spot_df.dropna(subset=['code', 'name', 'trade', 'turnoverratio', 'nmc'])
        spot_df = spot_df[~spot_df['name'].str.contains('ST|退|C|N|B')]
        
        # 步骤 B：游资活跃度过滤
        spot_df = spot_df[spot_df['turnoverratio'] >= 3.0]
        # 新浪流通市值(nmc)单位是万，500亿 = 500,0000万
        spot_df = spot_df[spot_df['nmc'] <= 5000000]
        
        # 步骤 C：按成交额 (amount) 降序，抽取最活跃的标的
        active_pool = spot_df.sort_values(by='amount', ascending=False).head(top_n)
        
        stock_list = active_pool['code'].tolist()
        name_dict = dict(zip(active_pool['code'], active_pool['name']))
        ...
        for _, row in active_pool.iterrows():
            spot_info_dict[row['code']] = {
                ...
                '换手率': row['turnoverratio'],
                '所属行业': '全网活跃主线'
            }
        
        print(f"✅ 新浪漏斗过滤完成！瞬间从全网浓缩至 {len(stock_list)} 只高频游资标的。")
        return stock_list, name_dict, spot_info_dict
    except Exception as e:
        print(f"⚠️ 新浪漏斗也被彻底拦截: {e}")
        fallback_list = ['600519', '000858', '300750']
        return fallback_list, {c: c for c in fallback_list}, {}

# 启动新浪漏斗
stock_list, name_dict, spot_info_dict = get_sina_active_pool(top_n=250)

# ==========================================
# ⚡️ 2. 核心算力：V6.0 主升浪打分引擎 (精算阶段)
# ==========================================
def process_single_stock(code):
    ...
    try:
        ...
        pct_change = spot_info.get('涨跌幅', today['pct_change'])
        turnover = spot_info.get('换手率', today.get('换手率', 0.0))
        industry = spot_info.get('所属行业', '活跃标的')
...
if len(results) > 0:
    ...
    summary_text += "🎯 【V6.0 新浪全网漏斗起爆点 Top 10】\n"
    summary_text += "-" * 45 + "\n"
