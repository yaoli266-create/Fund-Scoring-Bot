...
"
        content += f"当前全市场多头占比: {self.market_breadth:.1f}%
"
        content += "-" * 40 + "
"
        for _, row in df_top10.iterrows():
            content += f"【{row['综合评分']}分】{row['代码']} {row['名称']} | 现价:{row['最新价']} | 涨幅:{row['涨跌幅']}% | 换手:{row['换手率']}%
"
        ...
        try:
            # 默认使用QQ邮箱SMTP，如使用其他邮箱请修改此处服务器地址
